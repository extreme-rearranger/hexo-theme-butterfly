...
module.exports = function (hexo) {
    ...
    function getDisplayLanguages() {
        ...
        if (languages.indexOf('default') > -1) {
            // remove all 'default' from languages
            languages = languages.filter(language => language !== 'default');
        }
        return languages;
    }
    ...
    function isDefaultLanguage(language) {
        // return !language || getDisplayLanguages().indexOf(language) === 0;  // original
        return !language || getUsedLanguages().indexOf(language) === 0;  // changed
    }

    function postFilter(language) {  // if `lang` equals to filter language || `lang` or `language` is default language
        return function (post) {
            let lang = getPageLanguage(post);
            // return (lang === language || (isDefaultLanguage(language) && !lang)) && (post.indexing !== false);  // original
            return (lang === language || isDefaultLanguage(language) || isDefaultLanguage(lang)) && (post.indexing !== false);  // changed
